    ...
    stages {
        ...
        stage('remote db - create sample users') {
            steps {
                script {
                    FAILED_STAGE_NAME = "${STAGE_NAME}"
                    bat 'python src\\requests_api\\generate_new_users.py -t %REMOTE_DB_TEST_TYPE% -n 10'
                }
            }
        }
        ...
        stage('docker - create sample users') {
            steps {
                script {
                    ...
                    bat 'python src\\requests_api\\generate_new_users.py -t %DOCKER_TEST_TYPE% -n 10'
                }
            }
        }
            ...
            parallel {
                stage ('k8s - set ReplicaCount') {
                    steps {
                        script {
                            FAILED_STAGE_NAME = "${STAGE_NAME}"
                            bat 'python src\\admin\\set_replica_count_in_helm_values.py -r %REPLICA_COUNT%'
                        }
                    }
                }
         ...
         stage('k8s - create sample users') {
             steps {
                 script {
                     FAILED_STAGE_NAME = "${STAGE_NAME}"
                     sleep(time: 10, unit: "SECONDS")
                     bat 'python src\\requests_api\\generate_new_users.py -t %K8S_TEST_TYPE% -n 10'
                 }
             }
         }
